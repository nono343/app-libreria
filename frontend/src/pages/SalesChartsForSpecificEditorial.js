...
function SalesChartsForSpecificEditorial() {
  ...
  const formatComparisonData = (sales, period) => {
    ...
    const labels = [...new Set(sales.filter(s => s.editorial === editorial).map(sale => sale[period]))];

    const currentYearData = labels.map(label => {
      const sale = sales.find(s => s[period] === label && s.year == currentYear && s.editorial === editorial);
      return sale ? sale.total_sales : 0;
    });

    const lastYearData = labels.map(label => {
      const sale = sales.find(s => s[period] === label && s.year == lastYear && s.editorial === editorial);
      return sale ? sale.total_sales : 0;
    });
  ...
  return (
    <div>
      <h2>Sales Charts for Editorial: {editorial}</h2>
...
export default SalesChartsForSpecificEditorial;
