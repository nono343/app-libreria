...
function SalesChartsForEditorial() {
  ...
  useEffect(() => {
    const fetchComparisonData = async () => {
      try {
        const fetchWeeklyComparison = await fetch(`http://localhost:5000/api/saleseditorial/weekly/comparison`);
        ...
        const fetchMonthlyComparison = await fetch(`http://localhost:5000/api/saleseditorial/monthly/comparison`);
        ...
        const fetchYearlyComparison = await fetch(`http://localhost:5000/api/saleseditorial/yearly/comparison`);
  ...
  const aggregateSalesByEditorial = (sales) => {
    const salesMap = {};

    sales.forEach((sale) => {
      if (!salesMap[sale.editorial]) {
        salesMap[sale.editorial] = { editorial: sale.editorial, salesByYear: {} };
      }
      salesMap[sale.editorial].salesByYear[sale.year] =
        (salesMap[sale.editorial].salesByYear[sale.year] || 0) + sale.total_sales;
    });
  ...
  const formatComparisonData = (sales) => {
    const labels = [...new Set(sales.map((sale) => sale.editorial))];

    const currentYearData = labels.map((label) => {
      const sale = sales.find((s) => s.editorial === label && s.year == currentYear);
      return sale ? sale.total_sales : 0;
    });

    const lastYearData = labels.map((label) => {
      const sale = sales.find((s) => s.editorial === label && s.year == lastYear);
      return sale ? sale.total_sales : 0;
    });
  ...
  const formatPieData = (sales, period) => {
    ...
    const labels = [...new Set(filteredSales.map((sale) => sale.editorial))];
    const data = labels.map((label) => {
      const sale = filteredSales.find((s) => s.editorial === label);
      return sale ? sale.total_sales : 0;
    });
  ...
  const filteredSalesList = salesList.filter((sale) =>
    sale.editorial.toLowerCase().includes(searchTerm.toLowerCase())
  );
                ...
                <table className="min-w-full text-sm text-left text-gray-500">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredSalesList.map((sale, index) => (
                      <tr key={index}>
                        <td className="px-6 py-4 whitespace-nowrap">{sale.editorial}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <Link to={`/saleseditorial/${sale.editorial}`} className="text-indigo-600 hover:text-indigo-900">
                            View Sales Charts
                          </Link>
                        </td>
                      </tr>
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <h2 className="text-2xl font-bold text-center mb-6">Sales by Editorial</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        >
          ...
          <div className="p-5">
            ...
            <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">View the list of editorials and their sales.</p>
      ...
      <div className="mb-4">
        <input
          ...
          placeholder="Search by editorial"
...
export default SalesChartsForEditorial;
